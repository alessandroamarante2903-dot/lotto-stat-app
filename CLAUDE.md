...
### Riavvii periodici di `lotto_stat_web`: causa trovata, mitigata ma non eliminabile da codice

Il container si riavviava da solo ogni 30s–4min, `ExitCode=0`, nessun errore applicativo in log. Escluso con verifica diretta: OOM del kernel, `systemd-oomd`, AVC SELinux, `HEALTHCHECK` a livello immagine, file watcher di Streamlit (persisteva identico con `runOnSave=false`), comando di stop esplicito. Causa trovata aggiungendo un trap sui segnali nel `CMD` di `web/Dockerfile` (temporaneo, `sh -c 'trap ... TERM; streamlit run ... & wait $pid; echo uscito con codice $?'`):
- Prima occorrenza: **exit code 134 (SIGABRT)**, traceback che passava da `mysql/connector/connection_cext.py` — l'estensione C di `mysql-connector-python`, non un'eccezione Python normale.
- Dopo aver forzato `use_pure=True` (implementazione Python pura, niente estensione C) su tutte e tre le connessioni (`web/db.py`, `backend/db.py`, `backend/scraper.py`): il crash si è ripresentato uguale ma con **exit code 139 (SIGSEGV)**, segnale diverso.
- Segnale che cambia a ogni fix del componente nativo sospettato = sintomo di **pressione di memoria a livello HOST** (non del container: `podman stats` mostrava `lotto_stat_web` a ~130MB, ben sotto qualunque soglia), che fa fallire/corrompere in modo imprevedibile qualunque libreria con codice C tocchi per prima (numpy/pandas/plotly per l'UI, mysql-connector per il DB). Confermato: host con swap al 70-74% (`free -h`), a fronte di soli ~130MB usati dal container stesso — la pressione viene da altri processi sulla macchina (browser, IDE, ecc.), non da questo progetto.
- **Non risolvibile da codice applicativo**: non è un bug in `web/`, `backend/` o nelle query — è capacità di memoria della macchina host. Mitigazione applicata in `podman-compose.yml`: `mem_limit: 1g` (+ `memswap_limit: 1g`, anche se Podman lo ha applicato come 2g totale invece di 1g esatto — non perfetto ma comunque limitato) sul servizio `web`, così che se QUESTO container satura la sua memoria il kernel lo termina in modo pulito (SIGKILL, exit 137) invece di lasciarlo corrompere memoria in modi diversi ogni volta. Non elimina la causa di fondo (pressione host), la rende solo più prevedibile da questo lato.
- Il trap diagnostico in `web/Dockerfile` è stato lasciato attivo (utile per continuare a distinguere "crash nativo" da "stop esterno" in futuro); rimuoverlo se non serve più, tornando al `CMD` originale a singola riga senza `sh -c`/`trap`.
- **Rimedio reale**: liberare RAM sull'host (chiudere applicazioni, aggiungere RAM fisica) o spostare lo stack su una macchina con più memoria disponibile — non un compito per questo repository.
