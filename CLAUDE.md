...
Non esistono al momento suite di test, linter o build automatizzati in questo repo (eccetto i test manuali ad-hoc di `web/calcolo_costi.py`, senza framework, lanciati come script Python). Per verificare modifiche a `web/*.py` senza browser, il pattern usato in pratica è `streamlit.testing.v1.AppTest` lanciato ad-hoc dentro `lotto_stat_web` (nessun file di test permanente nel repo): `AppTest.from_file("/app/app.py")` seguito da `.switch_page("pages/N_..._Nome.py")` per testare una pagina nel vero contesto multipage — caricare una pagina isolata con `AppTest.from_file` direttamente sul file sotto `pages/` fa fallire ogni `st.page_link` con path relativo (`Could not find page`), perché la risoluzione dei path delle pagine avviene solo attraverso lo script di ingresso `app.py`.
...
- **`web/`** (container `lotto_stat_web`, Streamlit, tema dark "Control Room"): `app.py` è l'hub con 2 tab (Statistiche Live, Gestione & Scraper) — il Calcolatore & Sistemi è stato promosso da tab a pagina dedicata (vedi sotto). `db.py` (connessione MySQL condivisa con `st.cache_resource` + query cachate con `st.cache_data`, TTL 60s), `calcolo_costi.py` (combinatoria pura, nessuna dipendenza DB — vedi sotto). Monta anche `backend/` e `scraper/` per poter invocare lo scraping on-demand via `subprocess`. `.streamlit/config.toml` ha `runOnSave = true` e un blocco `[theme]` dark (`base="dark"`, palette ambra/blu scuro); Streamlit non è un'app ASGI (nessun `uvicorn --reload` applicabile), ma con il codice montato a volume `runOnSave` fa comunque ripartire la UI ad ogni modifica di un file, senza il prompt manuale di conferma.
  - **`web/pages/`**: 5 pagine Streamlit multipage, tutte condividono `filtri.py` (pannello parametri sidebar, session_state con chiavi `filtri_*`) e `carrello.py` (vedi sotto): `1_Analizzatore_Somme` (distribuzione somme vs teorica), `2_Simulatore_Backtest` (yield/max drawdown su una combinazione), `3_Tabellone_Analitico` (matrice 1-90, categorie/heatmap IRR), `4_Numero_Spia` (ranking Indice di Attrattiva), `5_Calcolatore_Sistemi` (ex tab di `app.py`: preventivo ADM, Sistema Integrale/Ridotto SuperEnalotto, costo Lotto — usa solo `calcolo_costi.py`, nessuna query DB).
  - **`web/ui_components.py`**: helper di presentazione condivisi fra `app.py` e tutte le pagine — `inject_custom_css()` (carica `web/style.css`), `render_header()` (testata con badge di stato), `render_lotto_ball()`/`render_balls_row()` (palline numerate), `apply_plotly_theme()` (tema dark uniforme sui grafici Plotly), `render_quick_nav()` (card di navigazione verso le 5 pagine).
  - **`web/carrello.py`**: carrello numeri condiviso fra le pagine di analisi e il Calcolatore, per evitare di ridigitare a mano i numeri "interessanti" visti in un'analisi. Due liste indipendenti in `session_state` (`carrello_lotto_numeri`, `carrello_sen_numeri` — Lotto e SuperEnalotto hanno vincoli diversi in `calcolo_costi.py`, nessun carrello "misto"). `get_carrello()` ritorna sempre una copia difensiva. `render_carrello_status()` è il banner riusabile (chiamato da ogni pagina) con conteggio, pulsante "Svuota" e link a `5_Calcolatore_Sistemi.py`. Nella pagina 5, un pulsante "Carica dal carrello" — posizionato nel sorgente **prima** del rispettivo `st.multiselect(key=...)` — precompila la selezione assegnando `st.session_state[key]` prima che il widget venga istanziato in quel run (vincolo Streamlit: va fatto in quest'ordine, o solleva `StreamlitAPIException`); l'utente resta comunque libero di modificare la selezione dopo.
...
- **Widget interattivi dentro un blocco `if st.button(...):` spariscono al primo rerun successivo**: pattern usato in tutto `web/` (Backtest, Numero Spia, Calcolatore) per calcolare-e-mostrare un risultato solo dopo un click. Va bene finché il blocco risultati contiene solo output statico (metriche, grafici, dataframe): al rerun successivo il bottone torna "non cliccato in questo run" e il blocco semplicemente non si ridisegna, senza effetti collaterali. Riscontrato in pratica aggiungendo controlli **interattivi** dentro quel blocco (es. un `st.number_input`/`st.selectbox` per configurare un pulsante "aggiungi al carrello" in `4_🕵️_Numero_Spia.py` e `2_🎲_Simulatore_Backtest.py`, vedi `web/carrello.py` sopra): cambiare uno di quei widget innesca un rerun in cui il bottone originale (`🔍 Avvia ricerca spia` / `🔎 Esegui backtest storico`) risulta di nuovo non cliccato, quindi l'intero blocco — inclusi i controlli appena toccati — sparisce, e l'interazione sembra "non fare nulla". Fix: al click, salvare il risultato in `st.session_state` (insieme ai parametri usati per le etichette, es. `spia_risultato`/`spia_risultato_label`, `backtest_risultato`/`backtest_numeri_usati`); il blocco di rendering legge sempre da lì (`risultato = st.session_state.get(...)`), non dal valore di ritorno del bottone in quel preciso run — così sopravvive a qualunque rerun causato dai controlli annidati.
- **`st.column_config.DateColumn` mostra l'epoch raw (es. `1789603200000`) invece della data formattata** su colonne pandas `object` contenenti `datetime.date` (non `datetime64[ns]`) — riscontrato in `web/app.py`, tabella "Stato archivio" del pannello Gestione & Scraper (`MAX(data_estrazione)` via `mysql.connector` torna `datetime.date` puro, non un dtype pandas nativo). Il badge di testata nella stessa pagina mostrava invece la data corretta perché costruito con un semplice f-string Python (`str()` su `datetime.date` funziona sempre), mascherando il problema per chi guardava solo l'header. Sospetta interazione con la stessa combinazione pyarrow 17.0.0/streamlit 1.38.0 già nota per il bug numpy 2.x qui sopra, ma non confermata a fondo (a differenza di quel caso). Fix: convertire esplicitamente la colonna a stringa in Python (`.map(lambda v: str(v) if v is not None else None)`, non `.astype(str)` — trasformerebbe i `None` nella stringa letterale `"None"`) prima di passarla a `st.dataframe`, e usare `TextColumn` invece di `DateColumn`.
- **`web/pages/3_🔢_Tabellone_Analitico.py` con Ruota = "Tutte": heatmap IRR vuota e colonne IRR/Max storico a `None`, senza alcun avviso** — l'Indice di Ritardo Relativo è per costruzione specifico di una ruota (il record storico di un numero è diverso ruota per ruota), esattamente come il Simulatore Backtest già documentava esplicitamente con un banner (`2_🎲_Simulatore_Backtest.py`). Il Tabellone aveva lo stesso vincolo ma senza il banner equivalente, quindi il risultato vuoto sembrava un errore anziché un vincolo noto. Fix: stesso pattern del Backtest — fallback silenzioso alla ruota "Napoli" (`ruota_tabellone = f.ruota if f.ruota != "Tutte" else "Napoli"`) più `st.info(...)` esplicito quando `f.ruota == "Tutte"`.
- **`web/.streamlit/config.toml` aveva `runOnSave = false` e `fileWatcherType = "none"`, in contraddizione col proprio commento in testa al file e con la sezione "Comandi comuni" qui sopra** (che documentano entrambe hot-reload attivo su modifica di un `.py` montato). Era già così nel commit corrente al momento della scoperta, non una regressione di questa sessione — ma il sintomo era subdolo: modifiche a `web/app.py` (es. il fix del bug `DateColumn` sopra) non si riflettevano mai nel browser nemmeno dopo un refresh completo della pagina, mentre `streamlit.testing.v1.AppTest` lanciato manualmente nel container mostrava il codice aggiornato corretto — la discrepanza fra i due è stato l'indizio che ha portato a controllare `config.toml` invece di sospettare un bug nel fix stesso. Fix: `runOnSave = true`, rimossa la riga `fileWatcherType`. **Essendo letto solo all'avvio del processo Streamlit, il fix richiede un `podman-compose up -d --force-recreate web` per attivarsi** — un semplice refresh o un altro salvataggio file non bastano.
